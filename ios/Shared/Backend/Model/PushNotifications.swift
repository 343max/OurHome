...
class PushNotificationSync {
  ...
  var deviceToken: String? {
    willSet {
      if let deviceToken = deviceToken, deviceToken != newValue {
        deletePushNotificationRegistration()
      }
    }
    didSet {
  ...
  init() {
    self.observer = Defaults.observe(keys: .doorbellRingPushNotification, .whenOtherUserArrivesPushNotification, options: [], handler: { [weak self] in
